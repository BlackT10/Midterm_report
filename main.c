...
void fsm_simple_buttons_run () {
	if(HAL_GPIO_ReadPin(DEC_GPIO_Port, DEC_Pin) == GPIO_PIN_SET){
		count_down(number);
	}
	else if(HAL_GPIO_ReadPin(INC_GPIO_Port, INC_Pin) == GPIO_PIN_SET){
		count_up(number);
	}
	else if(HAL_GPIO_ReadPin(RESET_GPIO_Port, RESET_Pin) == GPIO_PIN_SET){
		number=0;
	}
	else number=0;
}
